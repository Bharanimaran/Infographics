...
def plot_line_avg_attack_by_age(df, ax):
    ...
    sns.lineplot(x=avg_attack_by_age.index,
                 y=avg_attack_by_age.values, marker='o', color='black',
                 linestyle='-', linewidth=2, label='Average Attack Score',
                 ax=ax)
...
def plot_horizontal_bar_avg_attack_block_by_country(df, ax, top_countries=10):
    """
    Plots a horizontal bar chart for top countries based on average attack and 
    block scores.
    ...
    merged_df = pd.concat([avg_attack_by_country, avg_block_by_country], 
                          axis=1)
    merged_df.columns = ['Average Attack', 'Average Block']
    sns.barplot(data=merged_df.reset_index(), y='Country', x='Average Attack',
                color='skyblue', label='Average Attack', ax=ax)
    sns.barplot(data=merged_df.reset_index(), y='Country', x='Average Block', 
                color='orange', label='Average Block', ax=ax)
    ax.set_title(f'Top {top_countries} Countries: Average Attack and Block',
...
def plot_piechart_position_distribution(df, ax):
    ...
    wedges, texts, autotexts = ax.pie(position_distribution, 
    ...
    legend = ax.legend(wedges, position_distribution.index, title='Positions',
                       loc='center left', bbox_to_anchor=(1, 0, 0.5, 1), 
                       fontsize=18, facecolor='#F8F8F8')  
...
def analyze_player_statistics(df):
    """
    Analyzes player statistics and creates an infographic with multiple 
    subplots.
    ...
    plot_horizontal_bar_avg_attack_block_by_country(df, axs[0, 1],
                                                    top_countries=10)
    plot_piechart_position_distribution(df, axs[1, 0])
    plot_histogram_age_distribution(df, axs[1, 1])

    plt.suptitle('Player Statistics Analysis', fontsize=24, weight='bold',
                 y=1.02)
    ...
    plt.figtext(0.92, 0.5, description_text, ha='left', va='center', 
                fontsize=18, wrap=True, backgroundcolor='#F0F0F0',
                bbox=dict(facecolor='#F0F0F0', edgecolor='none', 
                          boxstyle='round,pad=0.5'))

    #plt.savefig("22076992.png", dpi=300, bbox_inches='tight',
    #            facecolor='#F0F0F0')
